...
# @Date : 28/02/2019
#
# This script is to setup local environment for shopcx - Draft
# @requirement
    ...
    do
        ...
        case "$version" in
            ...
            monitoring-dashboard*)
               local MONITORING_DASHBOARD=$(echo ${version} | sed -e "s/statemachine-api-dashboard-monitoring://")
...
#    cd $AUTOMATION_PATH && git pull
    ...
    echo "INFO > gradle clean automationTest -Pcategory=@RequirementData -Pnocategory=~@NotImplemented -Penvironment=docker -Pport=80 -PhazelcastAddress=hazelcast -PisUsingtoken=true -PisUsingAutomationPopDb=false"
    gradle clean automationTest -Pcategory=@RequirementData -Pnocategory=~@NotImplemented -Penvironment=docker -Pport=80 -PhazelcastAddress=hazelcast -PisUsingtoken=true -PisUsingAutomationPopDb=false
   ...
   cd $AUTOMATION_PATH
    ...
    echo "INFO > gradle clean automationTest -Pcategory=@ShopCXStructure -PisBasicAuthForShopCX=true -PisUsingAutomationPopDb=false -Puser=REDroot1 -POrganizationShopCX=RED -Pnocategory=~@NotImplemented -PisUsingAutomationPopDb=false -PcleanAllEntities=false"
    gradle clean automationTest -Pcategory=@ShopCXStructure -PisBasicAuthForShopCX=true -PisUsingAutomationPopDb=false -Puser=REDroot1 -POrganizationShopCX=RED -Pnocategory=~@NotImplemented -PisUsingAutomationPopDb=false -PcleanAllEntities=false

    echo "INFO > gradle clean automationTest -Pcategory=@EnableTransformRule -Pnocategory=~@NotImplemented -Penvironment=docker -PnumberCores=1 -Pport=80 -PhazelcastAddress=hazelcast -PisUsingtoken=true -PisUsingAutomationPopDb=false -POrganizationShopCX=RED -PcleanAllEntities=false"
    gradle clean automationTest -Pcategory=@EnableTransformRule -Pnocategory=~@NotImplemented -Penvironment=docker -PnumberCores=1 -Pport=80 -PhazelcastAddress=hazelcast -PisUsingtoken=true -PisUsingAutomationPopDb=false -POrganizationShopCX=RED -PcleanAllEntities=false
    ...
    gradle clean automationTest -Pcategory=@ShopCXStructureFixture -PisBasicAuthForShopCX=true -PisUsingAutomationPopDb=false -Puser=REDroot1 -POrganizationShopCX=RED -Pnocategory=~@NotImplemented -Penvironment=docker -Pport=80 -PcleanAllEntities=false
}
...
GIT_USER="<USER>"
GIT_PASSWORD="<PWD>"
ADD_CONTAINERS=shopcx

rm -rf $VIZIX_COMPOSE

 if [ -z $1 ]; then
    DOCKER_BRANCH_UI=dev_6.x.x
 else
    DOCKER_BRANCH_UI=$1
 fi

 if [ -z $2 ]; then
    DOCKER_BRANCH_BRIDGES=dev_6.x.x
 else
    DOCKER_BRANCH_BRIDGES=$2
 fi

 if [ -z $3 ]; then
    DOCKER_BRANCH_SERVICES=dev_6.x.x
 else
    DOCKER_BRANCH_SERVICES=$3
 fi

 if [ -z $4 ]; then
    DOCKER_BRANCH_TOOLS=dev_6.x.x
 else
    DOCKER_BRANCH_TOOLS=$4
 fi

 if [ -z $5 ]; then
    DOCKER_MICROSERVICES=configuration-api-devices:v6.55.0,configuration-api-languages:v6.55.0,dp-asn-auto-engine:v6.55.0,dp-instant-event-generation:v6.55.0,dp-retroactive-event-generation:v6.55.0,red-amqp-servicebus:v6.55.0,serialization-api:v6.55.0,vizix-api-transformer:v6.55.0,vizix-tenant-versioning:v6.55.0,report-generator:v6.55.0,sysconfig:dev/6.x.x,monitoring-api:v6.55.0,reports:v6.55.0,statemachine-api-dashboard-configuration:v6.55.0,epcis-event-generator:v6.55.0
 else
...
setupEnvironmentLocalShopCX $PATH_REPOSITORIES $GIT_USER $GIT_PASSWORD $AUTOMATION_PATH $VIZIX_COMPOSE $DOCKER_BRANCH_UI $DOCKER_BRANCH_BRIDGES $DOCKER_BRANCH_SERVICES $PUBLIC_IP $POPDB_NAME $DOCKER_BRANCH_TOOLS $DOCKER_MICROSERVICES
